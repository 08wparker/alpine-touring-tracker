...
const BERNER_BOUNDS = { minLat: 46.4, maxLat: 46.7, minLng: 7.8, maxLng: 8.3 }

function isInBerner(activity: StravaActivity): boolean {
  const [lat, lng] = activity.start_latlng || [0, 0]
  return lat >= BERNER_BOUNDS.minLat && lat <= BERNER_BOUNDS.maxLat &&
         lng >= BERNER_BOUNDS.minLng && lng <= BERNER_BOUNDS.maxLng
}
...
export default function BernerOberland() {
  ...
  useEffect(() => {
    getTourNames('berner-oberland').then(setTourNames).catch(console.error)
  }, [])
  ...
  useEffect(() => {
    getAllUsersActivities().then(allUsers => {
      ...
      const trackGroups: UserTrackGroup[] = allUsers.map((user: UserWithActivities, idx: number) => {
        const bernerActivities = user.activities.filter(isInBerner)
        const tracks: DecodedTrack[] = bernerActivities
      ...
      // Collect all Berner activities for photo interpolation
      const allActs: ActivityForPhoto[] = allUsers.flatMap(user =>
        user.activities.filter(isInBerner)
          .filter(a => a.map?.summary_polyline)
          .map(a => ({
            id: a.id,
            name: a.name,
      ...
      if (session?.user?.id) {
        ...
        if (currentUser) {
          const bernerActs = currentUser.activities.filter(isInBerner)
  ...
  const saveDayName = async (date: string) => {
    ...
    try {
      await saveTourName('berner-oberland', date, name)
      setTourNames(prev => {
